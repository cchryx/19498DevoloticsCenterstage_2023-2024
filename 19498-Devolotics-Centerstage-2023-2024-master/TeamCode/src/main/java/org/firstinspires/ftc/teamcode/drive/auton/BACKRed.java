...
@Autonomous
public class BACKRed extends OpMode {
    // Declaring Hardware
    ...
    Pose2d START_POSE = new Pose2d(12, -63, Math.toRadians(90));
//    Pose2d START_POSE = new Pose2d(34, -14, Math.toRadians(180));
...
//        imu.initialize(parameters);
    ...
    public void init_loop() {
        ...
        telemetry.addData("Can start","YES");
    ...
    public void start() {
        ...
        traj_right1 = drive.trajectoryBuilder(START_POSE)
                .splineToLinearHeading(new Pose2d(33.2, -36, Math.toRadians(180)), Math.toRadians(90))
                .build(); // spike mark

        traj_right2 = drive.trajectoryBuilder(traj_right1.end())
                .lineToLinearHeading(new Pose2d(50.3, -42.7, Math.toRadians(180)))
                .build(); // backdrop

        traj_right3 = drive.trajectoryBuilder(traj_right2.end())
                .lineToLinearHeading(new Pose2d(34, -14, Math.toRadians(184)))
                .build(); // line up to stack

        traj_right4 = drive.trajectoryBuilder(traj_right3.end())
                .lineToLinearHeading(new Pose2d(-56, -20, Math.toRadians(184)))
                .build(); // go stack

        traj_right104 = drive.trajectoryBuilder(traj_right4.end())
                .lineToLinearHeading(new Pose2d(-56.01, -20, Math.toRadians(184)))
                .build(); // stack

        traj_right5 = drive.trajectoryBuilder(traj_right104.end())
                .lineToLinearHeading(new Pose2d(25, -15, Math.toRadians(183)))
                .build(); // go backdrop

        traj_right6 = drive.trajectoryBuilder(traj_right5.end())
                .lineToLinearHeading(new Pose2d(51, -31.3, Math.toRadians(183)))
                .build(); // backdrop white

        traj_right7 = drive.trajectoryBuilder(traj_right6.end())
                .lineToLinearHeading(new Pose2d(39, -61.5, Math.toRadians(177)))
                .build();

        traj_right8 = drive.trajectoryBuilder(traj_right7.end())
                .lineToLinearHeading(new Pose2d(59, -63, Math.toRadians(177)))
                .build();
        ...
        traj_middle2 = drive.trajectoryBuilder(traj_middle1.end())
                .lineToConstantHeading(new Vector2d(50.5, -36.6))
                .build(); // backdrop

        traj_middle3 = drive.trajectoryBuilder(traj_middle2.end())
                .lineToLinearHeading(new Pose2d(34, -12, Math.toRadians(182)))
                .build(); // line up to stack

        traj_middle4 = drive.trajectoryBuilder(traj_middle3.end())
                .lineToLinearHeading(new Pose2d(-54, -14, Math.toRadians(182)))
                .build(); // go stack

        traj_middle104 = drive.trajectoryBuilder(traj_middle4.end())
                .lineToLinearHeading(new Pose2d(-54.01, -14, Math.toRadians(182)))
                .build(); // stack

        traj_middle5 = drive.trajectoryBuilder(traj_middle104.end())
                .lineToLinearHeading(new Pose2d(30, -14, Math.toRadians(182)))
                .build(); // go backdrop

        traj_middle6 = drive.trajectoryBuilder(traj_middle5.end())
                .lineToLinearHeading(new Pose2d(51, -26.3, Math.toRadians(182)))
                .build(); // backdrop white

        traj_middle7 = drive.trajectoryBuilder(traj_middle6.end())
                .lineToLinearHeading(new Pose2d(40, -66, Math.toRadians(182)))
                .build();

        traj_middle8 = drive.trajectoryBuilder(traj_middle7.end())
                .lineToLinearHeading(new Pose2d(59, -66, Math.toRadians(182)))
                .build();

        // Prop on the left
        traj_left1 = drive.trajectoryBuilder(START_POSE)
                .splineTo(new Vector2d(8, -36), Math.toRadians(140))
                .build(); // spike mark

//        traj_left1 = drive.trajectoryBuilder(START_POSE)
//                .lineToLinearHeading(new Pose2d(-54, -14, Math.toRadians(180)))
//                .build(); // go stack test

        traj_left2 = drive.trajectoryBuilder(traj_left1.end())
                .lineToLinearHeading(new Pose2d(51, -30.3, Math.toRadians(180)))
                .build(); // backdrop

        traj_left3 = drive.trajectoryBuilder(traj_left2.end())
                .lineToLinearHeading(new Pose2d(34, -14, Math.toRadians(186)))
                .build(); // line up to stack

        traj_left4 = drive.trajectoryBuilder(traj_left3.end())
                .lineToLinearHeading(new Pose2d(-54, -17, Math.toRadians(186)))
                .build(); // go stack

        traj_left104 = drive.trajectoryBuilder(traj_left4.end())
                .lineToLinearHeading(new Pose2d(-54.01, -17, Math.toRadians(186)))
                .build(); // stack

        traj_left5 = drive.trajectoryBuilder(traj_left104.end())
                .lineToLinearHeading(new Pose2d(40, -14, Math.toRadians(186)))
                .build(); // go backdrop

        traj_left6 = drive.trajectoryBuilder(traj_left5.end())
                .lineToLinearHeading(new Pose2d(51, -33.8, Math.toRadians(186)))
                .build(); // backdrop white

        traj_left7 = drive.trajectoryBuilder(traj_left6.end())
                .lineToLinearHeading(new Pose2d(38, -67, Math.toRadians(186)))
                .build();

        traj_left8 = drive.trajectoryBuilder(traj_left7.end())
                .lineToLinearHeading(new Pose2d(59, -69, Math.toRadians(186)))
                .build();
...
//            int arg3 = program.get(line)[3];
            ...
            switch (func) {
                ...
                case 8:
                    //Check Intaked
                    if (!detected) { //if both not detected
                        if (!fIn) { //if front not detected
                            switch (PROPLOCATION_N) { //move foward trajectory
                                case 0:
                        ...
                        else { //if front is detected
                            intakePos = intakeUp + 90; //move down one layer
                            if (timeout.milliseconds() > 3000) { //if more than 3s, move on
                                CHANGE_LINE = true;
                            }
                        }
                    } else {
